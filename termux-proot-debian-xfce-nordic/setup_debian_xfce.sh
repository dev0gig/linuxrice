...
# task-german-desktop (zieht LibreOffice + GDAL, ~500 MB). Stattdessen nur explizit thunar
# (FileManager) und xfce4-terminal. Keine Panel-Anpassungen — XFCE nutzt sein Standard-Panel.
...
# 4c. Panel-Anpassung wird NICHT automatisch angewendet.
# Grund: Die Panel-XML mit ihren vielen Anführungszeichen war bei der Übertragung über GitHub
# fehleranfällig und hat den Hauptlauf mehrfach gebrochen. Daher — wie Tailscale — als
# optionaler Befehl 'setup-panel' ausgelagert. Nach dem Setup im Debian einmalig aufrufen.
# ROBUST: Alle folgenden Heredocs sind SINGLE-quoted ('PANEL_EOF' etc.), dadurch bleiben die
# Anführungszeichen im XML unverändert und müssen NICHT escaped werden — das war die alte
# Fehlerquelle. Diese Variante ist immun gegen das Quote-Problem.
echo "📌 Lege optionalen Panel-Installer als Befehl 'setup-panel' an..."
proot-distro login debian -- bash -c 'cat > /usr/local/bin/setup-panel << '"'"'PANEL_OUTER_EOF'"'"'
#!/usr/bin/env bash
# Optionaler Panel-Installer. Bei Bedarf aufrufen: setup-panel
# Installiert benoetigte Plugins und schreibt die handgebaute Panel-Config + Launcher.
set -e
export DEBIAN_FRONTEND=noninteractive

echo "[1/4] Installiere benoetigte Panel-Plugins..."
apt install -y -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confold \
  xfce4-whiskermenu-plugin xfce4-clipman-plugin xfce4-notes-plugin

echo "[2/4] Schreibe Panel-Konfiguration..."
mkdir -p /root/.config/xfce4/xfconf/xfce-perchannel-xml
cat > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml << '\''PANELXML_EOF'\''
...
<channel name="xfce4-panel" version="1.0">
  ...
  <property name="plugins" type="empty">
    ...
    <property name="plugin-8" type="string" value="clock">
      ...
      <property name="digital-time-font" type="string" value="Sans 14"/>
      <property name="mode" type="uint" value="2"/>
    ...
    <property name="plugin-20" type="string" value="thunar-tpa"/>
    ...
    <property name="plugin-30" type="string" value="launcher">
      <property name="items" type="array">
        <value type="string" value="17838390665.desktop"/>
      </property>
    ...
    <property name="plugin-32" type="string" value="launcher">
      <property name="items" type="array">
        <value type="string" value="17838391717.desktop"/>
      </property>
    </property>
    <property name="plugin-33" type="string" value="launcher">
      <property name="items" type="array">
        <value type="string" value="17838392258.desktop"/>
      </property>
    </property>
    <property name="plugin-34" type="string" value="launcher">
      <property name="items" type="array">
        <value type="string" value="17838392639.desktop"/>
      </property>
    </property>
  </property>
</channel>
...
echo "[3/4] Schreibe Launcher-Dateien..."
mkdir -p /root/.config/xfce4/panel/launcher-30 /root/.config/xfce4/panel/launcher-32 \
         /root/.config/xfce4/panel/launcher-33 /root/.config/xfce4/panel/launcher-34

cat > /root/.config/xfce4/panel/launcher-30/17838390665.desktop << '\''L30_EOF'\''
[Desktop Entry]
Version=1.0
Type=Application
...
cat > /root/.config/xfce4/panel/launcher-32/17838391717.desktop << '\''L32_EOF'\''
[Desktop Entry]
Type=Application
TryExec=alacritty
Exec=alacritty
...
Actions=New;
X-XFCE-Source=file:///usr/share/applications/Alacritty.desktop
...
cat > /root/.config/xfce4/panel/launcher-33/17838392258.desktop << '\''L33_EOF'\''
[Desktop Entry]
Name=Firefox ESR
...
cat > /root/.config/xfce4/panel/launcher-34/17838392639.desktop << '\''L34_EOF'\''
[Desktop Entry]
...
echo "[4/4] Setze Standardanwendungen (fuer exo-open-Launcher)..."
mkdir -p /root/.config/xfce4
cat > /root/.config/xfce4/helpers.rc << '\''HELPERS_EOF'\''
FileManager=thunar
...
echo ""
echo "Panel eingerichtet. Bitte XFCE neu starten (oder abmelden/anmelden),"
echo "damit das Panel neu geladen wird. Alternativ im laufenden XFCE:"
echo "  xfce4-panel -r"
PANEL_OUTER_EOF
chmod +x /usr/local/bin/setup-panel
'
...
if curl -fLo /tmp/SpaceMono.zip \"$ASSET_BASE/SpaceMono.zip\"; then
  ...
  # Systemweite fontconfig-Regel: den generischen Alias 'monospace' UND 'sans-serif'
  # auf SpaceMono biegen. So nutzt JEDE App SpaceMono, auch solche außerhalb von XFCE,
  # die die xsettings-Config nicht lesen (z.B. GTK-Programme mit generischem Font-Wunsch).
  mkdir -p /etc/fonts/conf.d
  cat << 'FCEOF' > /etc/fonts/local.conf
<?xml version=\"1.0\"?>
<!DOCTYPE fontconfig SYSTEM \"fonts.dtd\">
<fontconfig>
  <alias>
    <family>monospace</family>
    <prefer><family>SpaceMono Nerd Font Mono</family></prefer>
  </alias>
  <alias>
    <family>sans-serif</family>
    <prefer><family>SpaceMono Nerd Font</family></prefer>
  </alias>
  <alias>
    <family>serif</family>
    <prefer><family>SpaceMono Nerd Font</family></prefer>
...
<channel name=\"xsettings\" version=\"1.0\">
  ...
  <property name=\"Gtk\" type=\"empty\">
    ...
    <property name=\"FontName\" type=\"string\" value=\"SpaceMono Nerd Font 10\"/>
    <property name=\"MonospaceFontName\" type=\"string\" value=\"SpaceMono Nerd Font Mono 10\"/>
  </property>
</channel>
EOF
...
# 12. (Panel-Anpassungen bewusst ENTFERNT: kein Whisker-Menu, keine Launcher, keine
# Plugin-Configs, keine Standardanwendungen-Zuweisung. XFCE startet mit Standard-Panel.
# Grund: Die verschachtelte Panel-XML-Config war fehleranfällig bei der Übertragung und
# wurde nicht zuverlässig übernommen. Bewusste Entscheidung für ein sauberes, robustes
# Standard-XFCE. Panel kann bei Bedarf manuell in XFCE angepasst werden.)
...
# 15. (Tailscale-Autostart + Firefox-Proxy sind jetzt Teil des optionalen Befehls
# 'setup-tailscale' — siehe Schritt 4b. Hier bewusst nichts mehr, damit das Grundsystem
# ohne Tailscale sauber bleibt.)
...
echo "   ✓ Alacritty als Standard-Terminal, SpaceMono Nerd Font, Dark Theme"
...
echo "   ✓ XFCE mit Standard-Panel (keine Panel-Anpassungen — bewusst schlank gehalten)"
...
echo ""
echo "🔗  TAILSCALE (optional, NICHT automatisch installiert):"
echo "   Falls du deinen eigenen Server per SSH über Tailscale erreichen willst,"
echo "   im Debian einmalig ausführen:  setup-tailscale"
echo "   Das installiert Tailscale + Autostart + Firefox-Proxy in einem Rutsch."
echo "   Danach:  /root/start-tailscale.sh  und  tailscale up  (Login im Browser)."
echo ""
echo "⚠️  EINMALIGE MANUELLE SCHRITTE (nicht automatisierbar):"
echo "   • In der Termux:X11-App (nicht Termux selbst) unter Keyboard-Einstellungen:"
